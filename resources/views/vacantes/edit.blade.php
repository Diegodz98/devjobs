...
<h1 class="text-2xl text-center mt-10">Editar Vacante {{$vacante->titulo}}</h1>
    ...
    <form action="{{ route('vacantes.update', ['vacante'=>$vacante->id]) }}" class="max-w-lg mx-auto my-10" method="POST">
        ...
        <div class="mb-5">
            ...
            <input id="titulo" type="text"
                ...
                value=" {{$vacante->titulo}}" autocomplete="titulo" autofocus placeholder="Tiutlo de la Vacante">
    ...
    <input type="hidden" name="imagen" id="imagen" value="{{$vacante->imagen}}">
        ...
        <div class="mb-5">
            ...
            @endphp
            <lista-skills :oldskills="{{ json_encode($vacante->skills) }}" :skills="{{ json_encode($skills) }}"
             ></lista-skills>
            @error('skills')
            <span class="bg-red-100 border-l-4 border-red-500 text-red-700 w-full mt-5 text-sm" role="alert">
                <strong>{{ $message }}</strong>
